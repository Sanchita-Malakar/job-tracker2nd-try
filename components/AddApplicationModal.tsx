...
const STATUS_CONFIG: Record<ApplicationStatus, { color: string; glow: string; label: string; icon: string }> = {
...
export default function AddApplicationModal({ isOpen, onClose, onAdd }: AddApplicationModalProps) {
  ...
  const [status,    setStatus]    = useState<ApplicationStatus>("Applied"); // ← fix: typed as ApplicationStatus
  ...
  return (
    <>
      <style>{`
        ...
        .modal-field-row > * { animation: field-in 0.3s ease both; }
        ...
        .am-input-date::-webkit-calendar-picker-indicator {
          filter: invert(0.6) sepia(1) saturate(2) hue-rotate(190deg); cursor: pointer;
        }
      ...
      >
        ...
        >
          ...
          <div style={{
            ...
            borderRadius: "24px 24px 0 0", opacity: 0.9,
          }} />
            ...
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "12px" }}>
              <div>
                <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "6px" }}>
                  <div style={{ position: "relative", width: "10px", height: "10px" }}>
                    <div style={{ position: "absolute", inset: 0, borderRadius: "50%", background: cfg.color, boxShadow: `0 0 8px ${cfg.color}` }} />
                    <div style={{ position: "absolute", inset: 0, borderRadius: "50%", background: cfg.color, opacity: 0.5, animation: "pulse-ring 1.8s cubic-bezier(0.2,0,0.8,1) infinite" }} />
                  </div>
                  <span style={{ fontSize: "11px", fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: "rgba(255,255,255,0.35)", fontFamily: "'DM Sans', sans-serif" }}>
                    New Application
                  </span>
                </div>
                <h2 style={{
                ...
                {role.trim() && (
                  <p style={{ margin: "4px 0 0", fontSize: "13px", color: cfg.color, fontWeight: 600, fontFamily: "'DM Sans', sans-serif", opacity: 0.9 }}>
                    {role.trim()}
                  </p>
                )}
              </div>
              ...
              <button
                ...
                style={{
                  ...
                  color: "rgba(255,255,255,0.45)", fontSize: "16px", cursor: "pointer",
                  display: "flex", alignItems: "center", justifyContent: "center",
                  transition: "all 0.18s",
                }}
                onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = "rgba(239,68,68,0.15)"; (e.currentTarget as HTMLElement).style.borderColor = "rgba(239,68,68,0.4)"; (e.currentTarget as HTMLElement).style.color = "#f87171"; }}
                ...
                <div key={s} style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "6px", cursor: s < step ? "pointer" : "default" }} onClick={() => s < step && setStep(s)}>
                    <div style={{
                      ...
                      background: step >= s ? "linear-gradient(135deg, #4f8ef7, #a78bfa)" : "rgba(255,255,255,0.07)",
                    ...
                    <span style={{ fontSize: "12px", fontWeight: 600, fontFamily: "'DM Sans', sans-serif", color: step >= s ? "rgba(255,255,255,0.7)" : "rgba(255,255,255,0.25)", transition: "color 0.3s" }}>
                      {s === 1 ? "Basics" : "Details"}
                    </span>
                  </div>
                  {s < 2 && (
                    <div style={{ width: "32px", height: "1px", background: step > s ? "linear-gradient(90deg, #4f8ef7, #a78bfa)" : "rgba(255,255,255,0.1)", transition: "background 0.4s" }} />
                  )}
                </div>
              ))}
            </div>
          ...
          {/* ── STEP 1: Basics ── */}
          {step === 1 && (
            <div key="step1" style={{ padding: "24px 28px", display: "flex", flexDirection: "column", gap: "20px", animation: "step-slide-left 0.3s ease both" }}>
              {/* Company */}
              <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                ...
                <div style={{ position: "relative" }}>
                  ...
                  />
                  {company.trim() && (
                    <div style={{ position: "absolute", right: "14px", top: "50%", transform: "translateY(-50%)", width: "18px", height: "18px", borderRadius: "50%", background: "rgba(16,185,129,0.2)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "11px", color: "#34d399" }}>✓</div>
                  )}
                </div>
              </div>
          ...
          {/* ── STEP 2: Details ── */}
          {step === 2 && (
            <div key="step2" style={{ padding: "24px 28px", display: "flex", flexDirection: "column", gap: "20px", animation: "step-slide-right 0.3s ease both" }}>
              {/* Date */}
              <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
              ...
              <div style={{
                ...
                <div style={{ minWidth: 0 }}>
                  <div style={{ fontSize: "14px", fontWeight: 700, color: "#e8eaf2", fontFamily: "'DM Sans', sans-serif", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                    {company} — {role}
                  </div>
          ...
          {/* ── FOOTER ── */}
          <div style={{ padding: "20px 28px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "12px" }}>
            <button
            ...
            {step === 1 ? (
              <button
                ...
                style={{
                  ...
                  background: step1Valid ? "linear-gradient(135deg, #4f8ef7 0%, #3b7ef0 100%)" : "rgba(255,255,255,0.06)",
                  ...
                  fontSize: "14px", fontWeight: 700, cursor: step1Valid ? "pointer" : "not-allowed",
                  fontFamily: "'DM Sans', sans-serif",
                  boxShadow: step1Valid ? "0 4px 20px rgba(79,142,247,0.4)" : "none",
                  transition: "all 0.2s", display: "flex", alignItems: "center", gap: "6px",
                }}
                onMouseEnter={e => { if (!step1Valid) return; (e.currentTarget as HTMLElement).style.transform = "translateY(-2px)"; (e.currentTarget as HTMLElement).style.boxShadow = "0 8px 28px rgba(79,142,247,0.55)"; }}
                onMouseLeave={e => { (e.currentTarget as HTMLElement).style.transform = "translateY(0)"; (e.currentTarget as HTMLElement).style.boxShadow = step1Valid ? "0 4px 20px rgba(79,142,247,0.4)" : "none"; }}
              >
                Next →
              </button>
            ) : (
              <button
                ...
                style={{
                  ...
                  fontFamily: "'DM Sans', sans-serif",
                  boxShadow: submitted ? "0 4px 20px rgba(16,185,129,0.4)" : `0 4px 20px ${cfg.glow}`,
                  transition: "all 0.3s",
                  display: "flex", alignItems: "center", gap: "8px",
                  minWidth: "140px", justifyContent: "center",
                }}
                onMouseEnter={e => { if (submitted) return; (e.currentTarget as HTMLElement).style.transform = "translateY(-2px)"; (e.currentTarget as HTMLElement).style.boxShadow = `0 8px 28px ${cfg.glow}`; }}
                onMouseLeave={e => { (e.currentTarget as HTMLElement).style.transform = "translateY(0)"; (e.currentTarget as HTMLElement).style.boxShadow = `0 4px 20px ${cfg.glow}`; }}
              >
                {submitted ? (
                  <>
